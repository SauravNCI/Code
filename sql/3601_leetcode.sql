...
WITH t1 AS (
    SELECT 
        t.*, 
        driver_name, 
        distance_km / fuel_consumed AS feff
    FROM drivers d 
...
SELECT 
    t2.driver_id,
    t2.driver_name,
    t2.first_half_ef AS first_half_avg,
    t3.s_half_ef AS second_half_avg,
    ROUND(t3.s_half_ef - t2.first_half_ef, 2) AS efficiency_improvement
FROM t2 
JOIN t3 
    ON t2.driver_id = t3.driver_id
WHERE t3.s_half_ef > t2.first_half_ef
