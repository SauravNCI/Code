...
,t2 as(
    select employee_id,employee_name,department,week(meeting_date) as week_number, sum(duration_hours) as total_weekly_hours

    from t1 group by employee_id,week(meeting_date)
)

select employee_id,employee_name,department, count(*) as meeting_heavy_weeks
from t2 where total_weekly_hours>20 group by employee_id having count(*)>=2  
